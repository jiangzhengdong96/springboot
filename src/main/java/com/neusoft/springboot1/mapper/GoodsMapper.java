...
public interface GoodsMapper {
    ...
    @Select("select goods.g_id,goods.g_name,goods.g_intro,goods.g_num,goods.g_pic,goods.g_status,goods.addtime," +
            "goods.g_price,user.u_name,type.t_name from goods left join user" +
            " on goods.u_id=user.u_id left join type on goods.t_id=type.t_id where goods.g_status = 1  and goods.up_status = 1")
    List<Goods> getAllGoods1();
    @Select("select goods.g_id,goods.g_name,goods.g_intro,goods.g_num,goods.g_pic,goods.g_status,goods.addtime," +
            "goods.g_price,user.u_name,type.t_name from goods left join user" +
            " on goods.u_id=user.u_id left join type on goods.t_id=type.t_id where goods.g_status = 1 and goods.t_id = #{tId}   and goods.up_status = 1")
    ...
    @Select("select goods.g_id,goods.g_name,goods.g_intro,goods.t_id,goods.g_num,goods.g_pic,goods.g_status,goods.addtime," +
            "goods.g_price,user.u_name,type.t_name from goods left join user" +
